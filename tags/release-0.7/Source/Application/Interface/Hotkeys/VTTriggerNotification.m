...
- (void) insertObjectInTriggers: (VTTrigger*) trigger atIndex: (unsigned int) triggerIndex {
  // now register the passed trigger and set it
  [trigger registerTrigger];
  [trigger setNotification: self];
  // and add to our list
  [mTriggers insertObject: trigger atIndex: triggerIndex];

  // and send notification
  [[NSNotificationCenter defaultCenter] postNotificationName: kVtNotificationWasRegistered object: self];
}

- (void) removeObjectFromTriggersAtIndex: (unsigned int) triggerIndex {
  VTTrigger* trigger = [mTriggers objectAtIndex: triggerIndex];
  ...
  [mTriggers removeObjectAtIndex: triggerIndex];
}
