...
#pragma mark -
- (void) bindDesktop: (PNDesktop*) desktop; 
- (void) unbindDesktop: (PNDesktop*) desktop; 
#pragma mark -
- (id) selectedItem; 
#pragma mark -
- (NSArray*) itemIdentifiers; 
#pragma mark -
- (NSAttributedString*) desktopDescription: (PNDesktop*) desktop; 
...
- (void) onDesktopDelete: (id) sender {
	...
	if ([selectedItem isKindOfClass: [PNDesktop class]] == NO)
		return; 
	
	PNDesktop* desktop	= (PNDesktop*) selectedItem; 
...
- (void) onDesktopInspect: (id) sender {
	...
	VTDesktopViewController* controller = [[VTDesktopViewController alloc] init];
	[controller window]; 
...
- (BOOL) validateToolbarItem: (NSToolbarItem*) item {
	if ([[item itemIdentifier] isEqualToString: VTDesktopDeleteIdentifier]) {
		...
		if ([[self selectedItem] isKindOfClass: [PNDesktop class]] == NO)
			return NO; 
...
- (id) outlineView: (NSOutlineView*) outlineView child: (int) childIndex ofItem: (id) item {
  ...
  //	}
	...
	if ([item isKindOfClass: [PNApplication class]]) {
		...
		return [applicationWindows objectAtIndex: childIndex]; 
	}
...
- (BOOL) outlineView: (NSOutlineView*) outlineView isItemExpandable: (id) item {
	...
	if ([item isKindOfClass: [PNDesktop class]]) 
		return ([[(PNDesktop*)item applications] count] > 0); 
	if ([item isKindOfClass: [PNApplication class]]) 
...
- (int) outlineView: (NSOutlineView*) outlineView numberOfChildrenOfItem: (id) item {
	...
	// desktop items... 
	if ([item isKindOfClass: [PNDesktop class]]) {
		NSArray* applications = [(PNDesktop*)item applications]; 
...
- (id) outlineView: (NSOutlineView*) outlineView objectValueForTableColumn: (NSTableColumn*) tableColumn byItem: (id) item {
	// desktop items 
	if ([item isKindOfClass: [PNDesktop class]]) {
		return [self desktopDescription: (PNDesktop*)item];  
	}
...
- (void) bindDesktop: (PNDesktop*) desktop {
	...
	[desktop addObserver: self forKeyPath: @"tags" options: NSKeyValueObservingOptionNew context: NULL]; 
...
- (NSAttributedString*) desktopDescription: (PNDesktop*) desktop {
	...
	NSMutableAttributedString* desktopDescriptor = [[[NSMutableAttributedString alloc] initWithString: [desktop displayName] attributes: desktopDescriptorAttr] autorelease]; 
...
- (NSAttributedString*) applicationDescription: (PNApplication*) application {
    ...
    NSDictionary* windowDescriptorAttr = [NSDictionary dictionaryWithObjectsAndKeys:[NSFont labelFontOfSize: [NSFont labelFontSize]], NSFontAttributeName,
																					[NSColor lightGrayColor], NSForegroundColorAttributeName, nil]; 
    NSAttributedString* windowDescriptor = [[[NSMutableAttributedString alloc] initWithString:windowDescription
																				   attributes:windowDescriptorAttr] autorelease]; 
