...
- (CTGradient *)removeColorStopAtIndex:(unsigned)colorIndex
  {
  CTGradient *newGradient = [self copy];
  CTGradientElement removedElement = [newGradient removeElementAtIndex:colorIndex];
  
  if(isnan(removedElement.position))
	[NSException raise:NSRangeException format:@"-[%@ removeColorStopAtIndex:]: index (%i) beyond bounds", [self class], colorIndex];
...
- (NSColor *)colorStopAtIndex:(unsigned)colorIndex
  {
  CTGradientElement *element = [self elementAtIndex:colorIndex];
...
- (CTGradientElement)removeElementAtIndex:(unsigned)colorIndex
  {
...
- (CTGradientElement *)elementAtIndex:(unsigned)colorIndex;			
  {
  unsigned count = 0;
  CTGradientElement *currentElement = elementList;
