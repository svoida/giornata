...
- (IBAction) setDesktopForWindow: (id) sender {
	...
	/* (optionally) activate selected desktop */
	if ([[NSUserDefaults standardUserDefaults] boolForKey:VTFollowWindowsOnMove] == YES)
...
- (void) setUpInterface {
	// popup button setup 
	NSArray*	desktops		= [[VTDesktopLayout sharedInstance] orderedDesktops]; 
	...
	for (menuItemIndex = 0; menuItemIndex < menuItemCount; menuItemIndex++) {
		PNDesktop*  desktop		= [desktops objectAtIndex: menuItemIndex]; 
		
		NSMenuItem *application = [[NSMenuItem alloc] initWithTitle:[desktop displayName]
															 action:@selector(setDesktopForApplication:)
													  keyEquivalent:[NSString stringWithFormat:@"%i", menuItemIndex + 1]];
		...
		[[mApplicationDesktopButton menu] addItem:application];
		[application release];

		NSMenuItem *window = [[NSMenuItem alloc] initWithTitle:[desktop displayName]
														action:@selector(setDesktopForWindow:)
												 keyEquivalent:[NSString stringWithFormat: @"%i", menuItemIndex + 1]];
		...
		[[mWindowDesktopButton menu] addItem:window];
		[window release];
	}
