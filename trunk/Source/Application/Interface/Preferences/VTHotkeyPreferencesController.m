...
- (id) outlineView: (NSOutlineView*) outlineView child: (int) childIndex ofItem: (id) item {
	if (item == nil) {
		...
		return [[[VTTriggerController sharedInstance] items] objectAtIndex: childIndex]; 
	}
	
	if ([item isKindOfClass: [VTTriggerGroup class]]) {
		// return notification from the hotkey group represented by the item 
		return [[item items] objectAtIndex: childIndex]; 
	}
...
- (IBAction) removeTrigger: (id) sender {
	...
	// remove the trigger 
	int triggerIndex = [[notification triggers] indexOfObject: trigger]; 
	[notification removeObjectFromTriggersAtIndex: triggerIndex]; 
...
- (IBAction) toggleTriggerType: (id) sender {
	...
	if ([sender tag] == 0) {
		...
		[notification insertObjectInTriggers: newTrigger atIndex: triggerIndex]; 
		
		// remove the old trigger 
		[notification removeObjectFromTriggersAtIndex: triggerIndex+1]; 
	}
